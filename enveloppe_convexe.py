...
print(f"generation: {elapsed_generation/nbre_repet:2.6f}")
print(f"calcul:     {elapsed_convexhull/nbre_repet:2.6f}")
print(f"total:      {sum((elapsed_generation, elapsed_convexhull))/nbre_repet:2.6f}")


#  validation de non-regression :
tailleTotal = 55440
if (taille_nuage == tailleTotal):
    ref = np.loadtxt(f"enveloppe_convexe_{tailleTotal}.ref")
    try:
        np.testing.assert_allclose(ref, enveloppe)
        print(f"verification: OK {tailleTotal}")
    except AssertionError as e:
        print(e)
        print(f"verification: FAILED {tailleTotal}")
